...
class NovoITEMActivity : AppCompatActivity(),RegistroFragment.OnFragmentInteractionListener {
    ...
    private var pessoaInserida: Pessoa? = null
    fun inicializar(){
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        supportFragmentManager.beginTransaction()
            .replace(R.id.IDF1_fragmentContainerView, RegistroFragment())
            .commit()
        inicializar()
        inicializaAcoes()
        obterDadosChegada()
    }
    override fun envioObjeto(objeto: Pessoa) {
        objeto?.getTel()?.setTelefone(telEditText?.text.toString())
        //// Colocar na lista no mesmo lugar do objeto anteriror
        listaChegada?.add(objeto) // Colocando a LISTA INTEIRA com o rotulo "LISTA2"
        var pacoteEnvio: Bundle = Bundle()
        pacoteEnvio.putSerializable("LISTA2",listaChegada)
        // Voltar para a Activity Principal
        val mainActivityIntent = Intent(this,MainActivity::class.java)
